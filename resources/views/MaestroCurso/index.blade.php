...
@section('content')
    ...
    <div class="container">
        ...
        <div class="card-header d-inline-flex">
            ...
            <a href="{{ url('MaestroCurso/pdf?search=' . request('search')) }}" class="btn btn-success"
                target="_blank">PDF</a>
        </div>
        <br>
        <div class="card mt-3">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        ...
                                        <a href="{{ url('/MaestroCurso/' . $detalle->id . '/edit') }}"
                                            class="btn btn-warning"><i class="fa fa-pencil-alt"></i></a>

                                        <form action="{{ url('/MaestroCurso/' . $detalle->id) }}" class="d-inline"
                                            method="post">
                                            @csrf
                                            {{ method_field('DELETE') }}
    ...
    <Script type="text/javascript">
        $('#limit').on('change', function() {
            window.location.href = "{{ route('MaestroCurso.index') }}?limit=" + $(this).val() + '&search=' + $(
                '#search').val()
        })

        $('#search').on('keyup', function(e) {
            if (e.keyCode == 13) {
                window.location.href = "{{ route('MaestroCurso.index') }}?limit=" + $('#limit').val() +
                    '&search=' + $(this).val()
            }
        })
    </Script>
