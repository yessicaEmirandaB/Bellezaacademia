                    ...
                    <td>
                        <div class="btn-group" role="group" aria-label="Basic example">
                            ...
                            <button type="button" class="btn btn-success" data-bs-toggle="modal"
                                ...
                                <i class="fa fa-money-bill-wave icon-size" aria-hidden="true"></i>
                            </button>
                            @include('AlumnoCurso.pagar-curso')

                            <form action="{{ route('AlumnoCurso.balance', ['id' => $detalle->id]) }}" method="POST">
                                @csrf
                                <button type="submit" class="btn btn-primary"><i class="fa fa-balance-scale icon-size"                                       target="_blank" aria-hidden="true"></i></button>
                            </form>


                            @can('editar-alumnoscursos')
                            <a href="{{ url('/AlumnoCurso/' . $detalle->id . '/edit') }}"
                                class="btn btn-warning"><i class="fa fa-pencil-alt icon-size"></i></a>
                            @endcan
                            ...
                            <form action="{{ url('/AlumnoCurso/' . $detalle->id) }}" class="d-inline"
                                ...
                                <button type="submit" class="btn btn-danger"
                                    onclick="return confirm('¿Desea eliminar?')" value="Borrar"><i
                                        class="fa fa-trash icon-size" aria-hidden="true"></i></button>
                            </form>
                            @endcan
                        </div>
                    </td>
...
<style>
.icon-size {
    font-size: 1rem; /* Ajusta el tamaño según lo necesites */
